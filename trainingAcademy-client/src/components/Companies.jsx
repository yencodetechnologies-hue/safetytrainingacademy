        ...
        try {
            ...
            if (editData) {
                const res = await axios.put(
                    `http://72.61.236.154:8000/api/companies/${editData._id}`,
                ...
                const res = await axios.post(
                    "http://72.61.236.154:8000/api/companies",
...
export default function Companies() {
    ...
    useEffect(() => {
        const fetchCompanies = async () => {
            try {
                const token = localStorage.getItem("token");
                const res = await axios.get("http://72.61.236.154:8000/api/companies", {
                    headers: { Authorization: `Bearer ${token}` },
                });
    ...
    const handleToggle = async (id) => {
        try {
            ...
            const res = await axios.patch(
                `http://72.61.236.154:8000/api/companies/${id}/toggle-status`,
                {},
                { headers: { Authorization: `Bearer ${token}` } }
            );
            setCompanies((prev) =>
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem("token");
            await axios.delete(`http://72.61.236.154:8000/api/companies/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
