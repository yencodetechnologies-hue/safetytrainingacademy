...
function DeleteModal({ link, onClose, onDelete }) {
  ...
  const handleDelete = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/enrollment-links/${link._id}`, { method: "DELETE" });
      const data = await res.json();
      if (!data.success) throw new Error(data.message);
      onDelete(link._id);
      onClose();
...
export default function EnrollmentLinks() {
  ...
  const handleToggleStatus = async (id) => {
    try {
      const res  = await fetch(`${API_URL}/api/enrollment-links/${id}/toggle-status`, { method: "PATCH" });
      const data = await res.json();
      if (!data.success) throw new Error(data.message);
